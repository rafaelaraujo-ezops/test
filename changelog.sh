...
git log $STAGING_HASH..$HASH -i -E --grep="^(feat)" --merges  --pretty=format:"**Author: %an** %n%n **commit message:** %s  %n%n %N %n">>wiki/changelog.md 
echo "### Fixes">>wiki/changelog.md
git log $STAGING_HASH..$HASH -i -E --grep="^(fix)"  --merges --pretty=format:"**Author: %an** %n%n **commit message:** %s  %n%n %N %n">>wiki/changelog.md
echo "### Styles">>wiki/changelog.md
git log $STAGING_HASH..$HASH -i -E --grep="^(style)" --merges  --pretty=format:"**Author: %an** %n%n **commit message:** %s  %n%n %N %n">>wiki/changelog.md
echo "### OPS">>wiki/changelog.md
git log $STAGING_HASH..$HASH -i -E --grep="^(ops)"  --merges --pretty=format:"**Author: %an** %n%n **commit message:** %s  %n%n %N %n">>wiki/changelog.md
